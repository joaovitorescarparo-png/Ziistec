...
export default async function handler(req,res){
  ...
  try{
    ...
    const pdf=await PDFDocument.create();pdf.setTitle(`Orçamento ${quote.number} - ${nomeCliente}`);pdf.setAuthor(company.trade_name||company.name||'ZiisTec');pdf.setSubject('Proposta comercial');pdf.setCreator('ZiisTec');
    ...
    let logoImg=null;
    const desenharMarcaDagua=(pg)=>{
      if(!logoImg)return;
      const {width:pw,height:ph}=pg.getSize();
      const alvo=Math.min(pw*0.62,ph*0.42);
      const escala=Math.min(alvo/logoImg.width,alvo/logoImg.height);
      const w=logoImg.width*escala,h=logoImg.height*escala;
      // É desenhada antes do conteúdo da página para ficar realmente ao fundo.
      pg.drawImage(logoImg,{x:(pw-w)/2,y:(ph-h)/2,width:w,height:h,opacity:0.05});
    };
    const addPage=()=>{page=pdf.addPage(A4);desenharMarcaDagua(page);y=A4[1]-44;};const ensure=(h)=>{if(y-h<50)addPage();};const text=(t,x,yy,size=10,font=normal,color=dark)=>page.drawText(clean(t),{x,y:yy,size,font,color});const wrapped=(t,x,maxW,size=10,font=normal,color=dark,lineH=14)=>{const lines=wrapText(t,font,size,maxW);ensure(lines.length*lineH+4);for(const line of lines){text(line,x,y,size,font,color);y-=lineH;}return lines.length;};

    if(company.logo_path){try{const encoded=String(company.logo_path).split('/').map(encodeURIComponent).join('/');const logoResp=await sbFetch(`/storage/v1/object/authenticated/zt-branding/${encoded}`,auth);const bytes=new Uint8Array(await logoResp.arrayBuffer()),ct=logoResp.headers.get('content-type')||'';if(ct.includes('png'))logoImg=await pdf.embedPng(bytes);else if(ct.includes('jpeg')||ct.includes('jpg'))logoImg=await pdf.embedJpg(bytes);if(logoImg){desenharMarcaDagua(page);const scale=Math.min(110/logoImg.width,48/logoImg.height,1);page.drawImage(logoImg,{x:margin,y:y-38,width:logoImg.width*scale,height:logoImg.height*scale});}}catch{}}
    ...
    text('ORÇAMENTO PARA',margin,y,9,bold,teal);y-=18;wrapped(nomeCliente,margin,width,15,bold,dark,18);const clientInfo=[client.tax_id&&`Documento: ${client.tax_id}`,client.contact_name&&`Contato: ${client.contact_name}`,client.phone&&`Tel.: ${client.phone}`,client.whatsapp&&`WhatsApp: ${client.whatsapp}`,client.address].filter(Boolean).join('  |  ');if(clientInfo)wrapped(clientInfo,margin,width,8.5,normal,muted,12);y-=12;
    ensure(40);page.drawRectangle({x:margin,y:y-18,width,height:22,color:light});text('Descrição',margin+6,y-12,8,bold,dark);text('Tipo',margin+262,y-12,8,bold,dark);text('Qtd.',margin+315,y-12,8,bold,dark);text('Unitário',margin+365,y-12,8,bold,dark);text('Total',margin+445,y-12,8,bold,dark);y-=30;let subtotal=0;
    ...
    for(const item of items||[]){const qty=Number(item.quantity||0),unit=Number(item.unit_price||0),total=qty*unit;subtotal+=total;const thumb=embeddedProductImages.get(item.product_id);const descX=thumb?margin+54:margin+6;const descW=thumb?252:300;const descLines=wrapText(item.name||'Item',normal,9,descW),noteLines=item.notes?wrapText(item.notes,normal,7.5,descW):[],rowH=Math.max(thumb?46:28,descLines.length*12+noteLines.length*10+8);ensure(rowH+6);if(thumb){const scale=Math.min(40/thumb.width,40/thumb.height,1);page.drawImage(thumb,{x:margin+6,y:y-40,width:thumb.width*scale,height:thumb.height*scale});}let yy=y;for(const line of descLines){text(line,descX,yy,9,normal,dark);yy-=12;}for(const line of noteLines){text(line,descX,yy,7.5,normal,muted);yy-=10;}text(item.kind==='product'?'Produto':item.kind==='service'?'Serviço':'Livre',margin+262,y,7.5,normal,muted);text(`${qty.toLocaleString('pt-BR')} ${clean(item.unit||'')}`,margin+315,y,8.5,normal,dark);text(money(unit),margin+365,y,8.5,normal,dark);text(money(total),margin+445,y,8.5,bold,dark);y-=rowH;page.drawLine({start:{x:margin,y:y+4},end:{x:A4[0]-margin,y:y+4},thickness:.5,color:light});}
    y-=8;ensure(100);const discount=Number(quote.discount||0),surcharge=Number(quote.surcharge||0),grand=Math.max(0,subtotal-discount+surcharge),lx=A4[0]-margin-210,vx=A4[0]-margin-95;text('Subtotal',lx,y,9,normal,muted);text(money(subtotal),vx,y,9,normal,dark);y-=15;if(discount>0){text('Desconto',lx,y,9,normal,muted);text(`- ${money(discount)}`,vx,y,9,normal,dark);y-=15;}if(surcharge>0){text('Acréscimo',lx,y,9,normal,muted);text(`+ ${money(surcharge)}`,vx,y,9,normal,dark);y-=15;}y-=6;page.drawRectangle({x:lx-12,y:y-12,width:A4[0]-margin-(lx-12),height:34,color:light});text('TOTAL',lx,y,12,bold,dark);text(money(grand),vx-14,y-2,17,bold,teal);y-=42;
    if(quote.payment_terms){text('CONDIÇÕES DE PAGAMENTO',margin,y,8,bold,teal);y-=14;wrapped(quote.payment_terms,margin,width,9,normal,dark,13);y-=7;}if(quote.notes){text('OBSERVAÇÕES',margin,y,8,bold,teal);y-=14;wrapped(quote.notes,margin,width,9,normal,dark,13);y-=7;}    const local=[quote.service_place,quote.address].filter(Boolean).join(' - ');
    if(local){text('LOCAL DO SERVIÇO',margin,y,8,bold,teal);y-=14;wrapped(local,margin,width,9,normal,dark,13);y-=7;}
    if(quote.valid_until){text('VALIDADE',margin,y,8,bold,teal);y-=14;wrapped(`Proposta válida até ${dateBR(quote.valid_until)}.`,margin,width,9,normal,dark,13);y-=7;}
    ensure(60);page.drawLine({start:{x:margin,y:y-4},end:{x:A4[0]-margin,y:y-4},thickness:.5,color:light});y-=18;
    const rodape=[companyName,company.tax_id&&`CNPJ/CPF ${company.tax_id}`,company.phone,company.whatsapp&&`WhatsApp ${company.whatsapp}`,company.email].filter(Boolean).join('  |  ');
    wrapped(rodape,margin,width,7.5,normal,muted,11);
    if(company.address)wrapped(company.address,margin,width,7.5,normal,muted,11);
    if(company.owner_name)wrapped(`Responsável: ${company.owner_name}`,margin,width,7.5,normal,muted,11);
    wrapped('Documento gerado pela ZiisTec.',margin,width,7,normal,muted,10);
    const bytes=await pdf.save(),filename=`${safeFile(quote.number)} - ${safeFile(nomeCliente)}.pdf`;res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`attachment; filename="${filename}"`);res.setHeader('Content-Length',String(bytes.length));return res.status(200).send(Buffer.from(bytes));
