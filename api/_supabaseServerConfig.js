...
export function resolverSupabaseServidor(env = process.env) {
  ...
  const isStagingPreview = vercelEnv === 'preview' && gitBranch === STAGING_BRANCH;
  ...
  // A branch de homologação recebe somente o projeto Supabase isolado/gratuito.
  if (isStagingPreview) {
    return {
      configurado: true,
...
export { PROD_SUPABASE_URL, STAGING_SUPABASE_URL, STAGING_BRANCH };
