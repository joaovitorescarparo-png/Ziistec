...
const rpcArgs=(company,wo,stage,hash,category='Antes',fileName='evidence.jpg')=>({
  p_company:company,p_work_order:wo,p_path:`${company}/work-orders/${wo}/${stage}/${hash}.jpg`,
  p_file_name:fileName,p_content_type:'image/jpeg',p_size_bytes:123,p_media_stage:stage,
  p_caption:null,p_category:category,p_content_sha256:hash,
});
...
const anon=await client.auth.signInAnonymously({options:{data:{rc1b_smoke_run:run}}});
if(anon.error||!anon.data.session) throw anon.error||new Error('ANON_SESSION_MISSING');
const userId=anon.data.user.id;
console.log(`RC1B_ANON_AUTH_READY user=${userId} run=${run}`);

let fixtureReady=false;
for(let attempt=1;attempt<=120;attempt++){
  const membership=await client.from('company_members').select('company_id,role,status').eq('company_id',COMPANY_A).eq('user_id',userId).maybeSingle();
  if(!membership.error&&membership.data?.status==='active'){fixtureReady=true;break;}
  if(attempt%10===0) console.log(`RC1B_WAIT_FIXTURE attempt=${attempt}`);
  await sleep(2000);
}
must(fixtureReady,'RC1B_FIXTURE_TIMEOUT');
console.log('RC1B_FIXTURE_READY');
...
const hSeq=sha(`seq:${run}`), hRace=sha(`race:${run}`), hOrphan=sha(`orphan:${run}`), hCross=sha(`cross:${run}`);
const result={userId,firstUpload:false,retryObject:false,retryMetadata:false,concurrency:false,sameId:false,finalCount:null,differentStage:false,sameCategoryDifferentStage:false,otherWorkOrder:false,crossTenantBlocked:false,orphanCleanup:false,ids:{}};
