...
export async function salvarOrcamentoDB(x, companyId, userId){
  ...
  const id=check(await supabase.rpc('zt_save_quote_idempotent',{p_company:companyId,p_quote:x.id||null,p_request:req,p_row:row,p_items:items}));
...
export async function salvarOSDB(x, companyId, userId){
  ...
  const req=requestIdFor(x);
  const id=check(await supabase.rpc('zt_save_work_order_idempotent',{p_company:companyId,p_wo:x.id||null,p_request:req,p_row:row,p_items:items}));
