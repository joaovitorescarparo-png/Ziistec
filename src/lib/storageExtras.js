...
export async function persistirFotosOSDB(osId,fotos,companyId,userId){
  for(const f of (fotos||[]).filter(x=>x?.arquivo instanceof File)){
    const category=f.categoria||'Foto';
    const stage=inferEvidenceStage(category);
    await persistWorkOrderEvidence({companyId,workOrderId:osId,file:f.arquivo,stage,category,caption:''});
  }
