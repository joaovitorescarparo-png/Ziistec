    ...
    <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-7">
      ...
      {loading?<div className="flex min-h-[45vh] items-center justify-center text-slate-500"><Loader2 className="mr-2 animate-spin" size={20}/>Carregando financeiro...</div>:<>
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[0.75fr_1.25fr]">
          ...
          <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col items-start gap-3 border-b border-slate-100 p-5 min-[390px]:flex-row min-[390px]:items-center min-[390px]:justify-between"><div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Rentabilidade por OS</p><p className="mt-1 text-xs text-slate-500">Receita lançada × custo privado real</p></div>{data.custosProntos?<span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700">Custos protegidos ativos</span>:<span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-700">Aguardando 0053–0054</span>}</div>
            {!data.custosProntos?<div className="m-5 rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-bold text-amber-900">Margem bloqueada no preview atual</p><p className="mt-1 text-xs leading-relaxed text-amber-800">Isso é proposital: enquanto os ledgers privados não estiverem homologados no banco, esta tela não usa `unit_cost` nem `extra_cost` das tabelas que o técnico pode consultar.</p></div>:osProfit.length===0?<div className="p-12 text-center text-sm text-slate-500">Nenhuma OS com movimento neste mês.</div>:<div className="divide-y divide-slate-100">{osProfit.slice(0,10).map(os=><div key={os.id} className="p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold">OS #{os.numero}</p><p className="mt-1 text-xs text-slate-500">{os.cliente} · {dataBR(os.concluidaEm||os.data)}</p></div><div className="text-right"><p className={`text-sm font-bold ${(os.resultado||0)>=0?'text-emerald-700':'text-rose-700'}`}>{brl(os.resultado)}</p><p className="text-[10px] text-slate-400">{os.margem==null?'sem receita':`${os.margem.toFixed(1)}% margem`}</p></div></div><div className="mt-3 grid grid-cols-1 gap-2 rounded-xl bg-slate-50 p-3 text-xs sm:grid-cols-3"><div><p className="text-[9px] font-bold uppercase text-slate-400">Receita</p><p className="mt-0.5 font-semibold">{brl(os.receita)}</p></div><div><p className="text-[9px] font-bold uppercase text-slate-400">Custo</p><p className="mt-0.5 font-semibold">{brl(os.custo)}</p></div><div><p className="text-[9px] font-bold uppercase text-slate-400">Resultado</p><p className="mt-0.5 font-semibold">{brl(os.resultado)}</p></div></div></div>)}</div>}
          </section>
        </div>

        <section className="mt-5 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 p-5"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Movimentos do mês</p><p className="mt-1 text-xs text-slate-500">Contas a receber e a pagar por vencimento</p></div>{recent.length===0?<div className="p-12 text-center text-sm text-slate-500">Nenhum lançamento neste mês.</div>:<div className="divide-y divide-slate-100">{recent.map(x=><div key={x.id} className="grid grid-cols-[auto_minmax(0,1fr)] gap-3 p-4 min-[390px]:grid-cols-[auto_minmax(0,1fr)_auto] min-[390px]:items-center sm:px-5"><div className={`rounded-xl p-2 ${x.tipo==='receita'?'bg-emerald-50 text-emerald-700':'bg-rose-50 text-rose-700'}`}>{x.tipo==='receita'?<ArrowDownRight size={17}/>:<ArrowUpRight size={17}/>}</div><div className="min-w-0 flex-1"><p className="break-words text-sm font-semibold">{x.descricao}</p><p className="mt-0.5 break-words text-[11px] text-slate-500">{clientMap.get(x.clienteId)||x.categoria||'Sem categoria'} · vence {dataBR(x.vencimento)} · {x.pago?'pago':'em aberto'}</p></div><strong className={`col-span-2 justify-self-end text-sm min-[390px]:col-span-1 min-[390px]:justify-self-auto ${x.tipo==='receita'?'text-emerald-700':'text-rose-700'}`}>{x.tipo==='receita'?'+':'−'} {brl(x.valor)}</strong></div>)}</div>}</section>
      </>}
    </main>
  </div>;
}
