...
import {
  ArrowLeft, CalendarDays, CheckCircle2, Loader2, RefreshCcw, ShieldCheck, Sparkles,
...
function Btn({ children, onClick, disabled=false, variant='primary', type='button', className='' }) {
  ...
  return <button type={type} onClick={onClick} disabled={disabled} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-3.5 py-2.5 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-50 ${cls} ${className}`}>{children}</button>;
}
...
const seedOrcamentoPago=(garantia,cliente)=>[
  'ATENDIMENTO FORA DA GARANTIA: esta cobertura está expirada e este deve ser um orçamento PAGO. Não trate como garantia gratuita, cortesia ou serviço sem cobrança.',
  cliente?.nome?`Cliente: ${cliente.nome}.`:null,
  garantia?.descricao?`Item/serviço relacionado: ${garantia.descricao}.`:null,
  garantia?.ate?`Garantia encerrou em ${dataBR(garantia.ate)}.`:null,
  garantia?.local?`Local do atendimento: ${garantia.local}.`:(cliente?.endereco?`Local do cliente: ${cliente.endereco}.`:null),
  garantia?.serie?`Número de série: ${garantia.serie}.`:null,
  'Monte uma proposta comercial para diagnóstico, reparo, manutenção ou substituição conforme necessário. Se o serviço exato, produto ou preço não estiver claro, marque como ambiguidade para o proprietário confirmar. Não invente preço e não use valor zero por causa da garantia antiga.',
].filter(Boolean).join(' ');

export default function ManualWarrantyV2({ companyId, companyName='Sua empresa', onClose, onQuoteFromWarranty }) {
  ...
  return <div className="min-h-[100dvh] overflow-x-hidden bg-slate-50 text-slate-900">
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-6"><div className="flex min-w-0 items-center gap-3"><button onClick={onClose} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-slate-200 p-2.5 text-slate-600 hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500" aria-label="Voltar"><ArrowLeft size={19}/></button><div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate text-base font-bold">Garantias manuais</h1><span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700">V2</span></div><p className="truncate text-xs text-slate-500">{companyName}</p></div></div><Btn variant="secondary" onClick={load} disabled={loading}><RefreshCcw size={16}/><span className="hidden sm:inline">Atualizar</span></Btn></div></header>

    <main className="mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-7">
      <div className="mb-5 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 shrink-0 text-emerald-700" size={20}/><div><p className="text-sm font-bold text-emerald-950">Registro do proprietário</p><p className="mt-1 text-xs leading-relaxed text-emerald-800">A criação manual exige permissão de proprietário no próprio RPC do banco. Cliente, produto e serviço também precisam pertencer à mesma empresa. Quando a cobertura vence, o histórico pode virar um orçamento pago sem reaproveitar a gratuidade da garantia.</p></div></div></div>
      {error&&<div className="mb-5 flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800"><TriangleAlert className="mt-0.5 shrink-0" size={18}/><div className="flex-1">{error}</div><button onClick={()=>setError('')} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-400" aria-label="Fechar erro"><X size={17}/></button></div>}
      {success&&<div className="mb-5 flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800"><CheckCircle2 className="mt-0.5 shrink-0" size={18}/><div className="flex-1">{success}</div><button onClick={()=>setSuccess('')} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500" aria-label="Fechar confirmação"><X size={17}/></button></div>}
