...
export default function ClientLocationsV2({companyId,companyName='Sua empresa',onClose,initialClientId=null}){
  ...
  return <div className="min-h-[100dvh] overflow-x-hidden bg-slate-950 text-white">
    <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/95 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-6"><div className="flex min-w-0 items-center gap-3"><button onClick={selectedId?()=>setSelectedId(null):onClose} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 p-2.5 text-slate-300 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300" aria-label="Voltar"><ArrowLeft size={19}/></button><div className="min-w-0"><h1 className="truncate text-base font-bold">Clientes e locais</h1><p className="truncate text-xs text-slate-500">{selected?.name||companyName}</p></div></div><button onClick={load} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 p-2.5 text-slate-300 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300" aria-label="Atualizar"><RefreshCw size={18}/></button></div></header>

    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      {error&&<div className="mb-4 flex items-start gap-3 rounded-2xl border border-rose-400/20 bg-rose-400/10 p-4 text-sm text-rose-200"><AlertTriangle className="mt-0.5 shrink-0" size={17}/><span>{error}</span><button onClick={()=>setError('')} className="ml-auto inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-300"><X size={16}/></button></div>}
      ...
      {selectedId&&selected&&<div className="mx-auto max-w-3xl space-y-5">
        <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 sm:p-6"><div className="flex flex-col gap-3 min-[390px]:flex-row min-[390px]:items-start min-[390px]:justify-between"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[.15em] text-sky-300">Cliente</p><h2 className="mt-1 text-xl font-bold">{selected.name}</h2>{selected.whatsapp&&<p className="mt-1 text-xs text-slate-500">WhatsApp {selected.whatsapp}</p>}</div>{(form.mapsUrl||currentMap)&&<button onClick={openMap} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-sky-400/20 bg-sky-400/10 px-3 py-2 text-xs font-bold text-sky-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300 min-[390px]:w-auto"><ExternalLink size={14}/>Abrir Maps</button>}</div>
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row"><button onClick={useGps} disabled={locating} className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.05] py-3 text-xs font-bold text-slate-300 hover:bg-white/[0.08] disabled:opacity-50"><Crosshair size={15}/>{locating?'Obtendo GPS...':'Usar localização atual'}</button><button onClick={openMap} disabled={!(form.mapsUrl||currentMap)} className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.05] py-3 text-xs font-bold text-slate-300 disabled:opacity-40"><MapPin size={15}/>Conferir no Google Maps</button></div>
