...
import {
  ...
  Mic, MicOff, Minus, PencilLine, Plus, RefreshCcw, Save, Sparkles, Trash2, UserPlus, X,
} from 'lucide-react';
...
function Button({children,onClick,disabled=false,type='button',variant='primary',className=''}) {
  ...
  return <button type={type} onClick={onClick} disabled={disabled} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-3.5 py-2.5 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-50 ${styles} ${className}`}>{children}</button>;
}
...
export default function QuoteAIV2({companyId,companyName='Sua empresa',userId,onClose,initialText=''}) {
  ...
  const save=async()=>{
    ...
    try{const quote=await salvarOrcamentoV2DB({...preview,mensagemCliente:customerMessage,previsaoExecucao:executionForecast,mostrarImagensProdutos:showProductImages},base,companyId,userId);setSaved(quote);}
    catch(e){setError(mensagemErro(e));}
  ...
  const newQuote=()=>{setText('');setInterim('');setPreview(null);setCorrection('');setAmbiguitiesReviewed(false);setSaved(null);setError('');setCustomerMessage('');setCustomerInterim('');setExecutionForecast('');setShowProductImages(false);customerVoice.stop?.();};

  if(loading)return <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 text-sm text-slate-500"><Loader2 className="mr-2 animate-spin" size={20}/>Carregando orçamento V2...</div>;

  return <div className="min-h-[100dvh] bg-slate-50 text-slate-900">
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6"><div className="flex min-w-0 items-center gap-3"><button onClick={onClose} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-slate-200 p-2.5 text-slate-600 hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"><ArrowLeft size={19}/></button><div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate text-base font-bold">Orçamento com IA</h1><span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700">V2</span></div><p className="truncate text-xs text-slate-500">{companyName}</p></div></div><Button variant="secondary" onClick={newQuote}><RefreshCcw size={15}/><span className="hidden sm:inline">Novo</span></Button></div></header>

    <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-7">
      {error&&<div className="mb-5 flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800"><AlertTriangle className="mt-0.5 shrink-0" size={18}/><span className="flex-1">{error}</span><button onClick={()=>setError('')} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-400" aria-label="Fechar erro"><X size={17}/></button></div>}
      ...
      <div className="grid gap-5 lg:grid-cols-[0.9fr_1.4fr]">
        <section className="space-y-4"><div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><div className="rounded-2xl bg-emerald-50 p-3 text-emerald-700"><Mic size={21}/></div><div><h2 className="text-sm font-bold">Fale como você trabalha</h2><p className="mt-1 text-xs leading-relaxed text-slate-500">Exemplo: “Orçamento para João, duas fechaduras Intelbras a 890 cada e instalação 400. Pix na entrega.”</p></div></div><textarea value={text} onChange={e=>setText(e.target.value)} rows={7} placeholder="Fale ou digite o pedido do cliente..." className="mt-4 w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-relaxed outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"/>{interim&&<p className="mt-2 rounded-xl bg-slate-100 px-3 py-2 text-xs italic text-slate-500">Ouvindo: {interim}</p>}{voice.processing&&<p className="mt-2 text-xs font-semibold text-sky-700">Transcrevendo...</p>}{voice.success&&<p className="mt-2 text-xs font-semibold text-emerald-700">Texto inserido.</p>}{voice.error&&<div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-amber-700"><span>{voice.error}</span><button type="button" onClick={voice.retry} className="min-h-11 rounded-lg border border-amber-200 px-3 font-bold">Tentar novamente</button></div>}<div className="mt-3 flex flex-col gap-2 sm:flex-row"><Button variant={voice.busy?'secondary':'soft'} onClick={voice.listening?voice.stop:voice.processing?voice.cancel:voice.start} disabled={!voice.supported}>{voice.listening?<><MicOff size={16}/>Parar ditado</>:voice.processing?<><Loader2 className="animate-spin" size={16}/>Transcrevendo...</>:<><Mic size={16}/>Ditado por voz</>}</Button>{voice.busy&&<Button variant="secondary" onClick={voice.cancel}>Cancelar</Button>}<Button onClick={()=>interpret(false)} disabled={!text.trim()||interpreting} className="flex-1">{interpreting?<Loader2 className="animate-spin" size={16}/>:<Sparkles size={16}/>}Interpretar orçamento</Button></div>{!voice.supported&&<p className="mt-2 text-[11px] text-slate-400">Seu navegador não oferece reconhecimento de voz; a digitação continua funcionando normalmente.</p>}</div>
        ...
        <section className="min-w-0">{!preview?<div className="flex min-h-[420px] items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center"><div><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-slate-500"><PencilLine size={25}/></div><h2 className="mt-4 text-base font-bold">A prévia aparece aqui</h2><p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-slate-500">Depois de interpretar, você revisa cliente, itens, quantidades, preços, custo e margem antes de salvar.</p></div></div>:
          <div className="space-y-4"><div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><Field label="Cliente"><div className="relative"><select value={preview.clienteId||''} onChange={e=>setPreview({...preview,clienteId:e.target.value||null})} className={`${inputClass} appearance-none pr-9`}><option value="">Selecione o cliente</option>{(base?.clientes||[]).map(c=><option key={c.id} value={c.id}>{c.fantasia||c.nome}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-slate-400" size={16}/></div></Field><Button variant="secondary" onClick={()=>setNewClient(true)}><UserPlus size={15}/>Cadastrar cliente</Button></div>{preview.resumo&&<p className="mt-3 rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-600">{preview.resumo}</p>}</div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><div className="grid gap-3 sm:grid-cols-2"><Field label="Desconto"><input type="number" min="0" step="0.01" value={preview.desconto} onChange={e=>setPreview({...preview,desconto:e.target.value})} className={inputClass}/></Field><Field label="Acréscimo"><input type="number" min="0" step="0.01" value={preview.acrescimo} onChange={e=>setPreview({...preview,acrescimo:e.target.value})} className={inputClass}/></Field></div><div className="mt-4 rounded-2xl border border-emerald-100 bg-emerald-50/40 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold text-slate-700">Mensagem ao cliente</p><p className="mt-1 text-[11px] leading-relaxed text-slate-500">Aparece no PDF antes dos produtos e serviços. Não substitui observações internas.</p></div><div className="flex flex-wrap gap-2"><Button variant={customerVoice.busy?'secondary':'soft'} onClick={customerVoice.listening?customerVoice.stop:customerVoice.processing?customerVoice.cancel:customerVoice.start} disabled={!customerVoice.supported}>{customerVoice.listening?<><MicOff size={16}/>Parar</>:customerVoice.processing?<><Loader2 className="animate-spin" size={16}/>Transcrevendo...</>:<><Mic size={16}/>Ditar mensagem</>}</Button>{customerVoice.busy&&<Button variant="secondary" onClick={customerVoice.cancel}>Cancelar</Button>}</div></div><textarea maxLength={5000} rows={4} value={customerMessage} onChange={e=>setCustomerMessage(e.target.value)} placeholder="Ex.: este orçamento contempla o fornecimento e a instalação completa conforme descrito abaixo." className={`${inputClass} mt-3 min-h-[110px] resize-y`}/>{customerInterim&&<p className="mt-2 text-xs italic text-slate-500">Ouvindo: {customerInterim}</p>}{customerVoice.processing&&<p className="mt-2 text-xs font-semibold text-sky-700">Transcrevendo...</p>}{customerVoice.success&&<p className="mt-2 text-xs font-semibold text-emerald-700">Texto inserido.</p>}{customerVoice.error&&<p className="mt-2 text-xs text-amber-700">{customerVoice.error}</p>}</div><div className="mt-3 grid gap-3 sm:grid-cols-2"><Field label="Previsão de execução (opcional)"><input type="date" value={executionForecast} onChange={e=>setExecutionForecast(e.target.value)} className={inputClass}/></Field><label className="flex min-h-11 items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-700 sm:self-end"><input type="checkbox" checked={showProductImages} onChange={e=>setShowProductImages(e.target.checked)} className="h-5 w-5 accent-emerald-700"/><span>Mostrar imagens dos produtos no PDF</span></label></div><Field label="Condição de pagamento"><input value={preview.condicao||''} onChange={e=>setPreview({...preview,condicao:e.target.value})} className={`${inputClass} mt-3`}/></Field><Field label="Observações"><textarea rows={3} value={preview.obs||''} onChange={e=>setPreview({...preview,obs:e.target.value})} className={`${inputClass} mt-3 resize-none`}/></Field><div className="mt-5 flex flex-col gap-4 border-t border-slate-100 pt-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold text-slate-500">Total para o cliente</p><p className="mt-1 text-3xl font-bold text-slate-900">{brl(totals.final)}</p></div><Button onClick={save} disabled={saving||currentIssues.length>0} className="sm:min-w-44">{saving?<Loader2 className="animate-spin" size={16}/>:<Save size={16}/>}Salvar rascunho</Button></div>{currentIssues.length>0&&<div className="mt-4 rounded-xl bg-amber-50 p-3 text-xs text-amber-800"><p className="font-bold">Antes de salvar:</p><ul className="mt-1 list-disc space-y-1 pl-4">{currentIssues.map(issue=><li key={issue}>{issue}</li>)}</ul></div>}</div>
          </div>}
        </section>
      </div>}
    </main>
