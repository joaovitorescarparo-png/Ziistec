...
create temp table zt_fw3b (
  ...
  cross_template_visible integer
) on commit drop;
...
-- -----------------------------------------------------------------------------
-- Owner A: duas OS e template com um item obrigatório e um opcional.
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000001',true);
set local role authenticated;
insert into public.clients(id,company_id,person_type,name,address)
select client_id,'20000000-0000-0000-0000-000000000001','PJ','CI Wave3B Cliente','Rua Checklist, 84' from zt_fw3b;
...
select wo_id,'20000000-0000-0000-0000-000000000001','CI-FW3B-001',client_id,
 '10000000-0000-0000-0000-000000000003','in_progress','Rua Checklist, 84','Porta social','Instalação Wave 3B' from zt_fw3b;
insert into public.work_orders(id,company_id,number,client_id,assigned_to,status,address,service_place,request)
select other_wo_id,'20000000-0000-0000-0000-000000000001','CI-FW3B-002',client_id,
 '10000000-0000-0000-0000-000000000001','in_progress','Rua Checklist, 84','Sala técnica','OS não atribuída ao técnico' from zt_fw3b;
...
begin
 ...
 if (select count(*) from public.work_order_checklists where work_order_id=t.wo_id)<>2 then raise exception 'Aplicação dupla duplicou checklist'; end if;
end $$;

-- Mudar template depois não muda snapshot da OS antiga.
...
begin
 select * into t from zt_fw3b;
 if (select count(*) from public.work_order_checklists where work_order_id=t.wo_id)<>2 then raise exception 'Template alterou quantidade da OS antiga'; end if;
 if exists(select 1 from public.work_order_checklists where work_order_id=t.wo_id and text='NOVO texto do template') then raise exception 'Template alterou snapshot da OS antiga'; end if;
end $$;
reset role;

-- Owner B cria template próprio.
...
-- Owner A não lê nem aplica template cross-tenant.
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000001',true);
set local role authenticated;
update zt_fw3b t set cross_template_visible=(select count(*) from public.checklist_templates x where x.id=t.other_template_id);
do $$ begin
...
-- -----------------------------------------------------------------------------
-- Técnico: não administra template; vê/marca somente checklist da OS atribuída.
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000003',true);
set local role authenticated;
do $$ begin
 perform public.zt_save_checklist_template('20000000-0000-0000-0000-000000000001',null,'Template indevido',null,true,'[{"text":"x"}]'::jsonb);
...
update public.work_order_checklists set done=true,text='TENTATIVA DE ALTERAÇÃO',required=false
 where work_order_id=(select wo_id from zt_fw3b) and required=true;
do $$
declare c public.work_order_checklists%rowtype;
begin
 select * into c from public.work_order_checklists where work_order_id=(select wo_id from zt_fw3b) and source_template_id is not null order by position limit 1;
 if not c.done then raise exception 'Técnico não marcou checklist atribuído'; end if;
 if c.text<>'alimentação testada' or not c.required then raise exception 'Técnico adulterou item de template'; end if;
end $$;
reset role;

-- Disabled technician perde acesso.
...
-- -----------------------------------------------------------------------------
-- Precisa retornar: idempotente, permanece aberta, sem financeiro/garantia/relatório final.
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000003',true);
set local role authenticated;
update zt_fw3b t set
...
begin
 select * into t from zt_fw3b; select * into w from public.work_orders where id=t.wo_id;
 ...
 if w.status='done' or w.completed_at is not null or not w.needs_return then raise exception 'Precisa retornar alterou estado definitivo incorretamente'; end if;
...
-- Assinatura cancelada bloqueia escrita; reativação restaura.
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000001',true);
set local role authenticated;
...
-- Obrigatoriedade é backend authority: finalização falha até marcar o item.
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000001',true);
set local role authenticated;
insert into public.work_order_checklists(work_order_id,company_id,text,done,position,created_by,required)
select other_wo_id,'20000000-0000-0000-0000-000000000001','Obrigatório pendente',false,0,'10000000-0000-0000-0000-000000000001',true from zt_fw3b;
do $$ begin
...
-- Conclusão posterior preserva retorno e registra data real.
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000003',true);
set local role authenticated;
...
begin
 select * into t from zt_fw3b; select * into w from public.work_orders where id=t.wo_id; select * into r from public.work_order_returns where id=t.return_id;
 if w.status<>'done' or w.completed_at is null then raise exception 'Conclusão posterior não concluiu OS'; end if;
 if w.needs_return then raise exception 'OS concluída permaneceu needs_return'; end if;
...
-- Asserções finais de autorização e projeção privada.
do $$
declare t zt_fw3b%rowtype; payload text;
begin
 select * into t from zt_fw3b;
 if t.cross_template_visible<>0 then raise exception 'Cross-tenant leu template'; end if;
