-- ZiisTec · gravações transacionais de orçamento, OS e compras
-- Mantém número + cabeçalho + itens (+ financeiro em compras) na mesma transação.
...
begin
  ...
  if p_quote is null then
    ...
    insert into public.quotes(
      ...
      payment_terms,notes,address,service_place,created_by
    ) values (
      p_company,v_number,nullif(p_row->>'client_id','')::uuid,
      ...
      nullif(p_row->>'payment_terms',''),nullif(p_row->>'notes',''),
      nullif(p_row->>'address',''),nullif(p_row->>'service_place',''),v_uid
    ...
    update public.quotes set
      client_id=nullif(p_row->>'client_id','')::uuid,
  ...
  for item in select value from jsonb_array_elements(coalesce(p_items,'[]'::jsonb)) loop
    insert into public.quote_items(
      quote_id,company_id,kind,service_id,product_id,name,unit,quantity,
      unit_price,unit_cost,notes,position
    ) values (
      v_id,p_company,
...
create or replace function zt_private.zt_save_work_order(
  p_company uuid,
  p_wo uuid,
  p_row jsonb,
  p_items jsonb default '[]'::jsonb
...
begin
  ...
  if p_wo is null then
    ...
    insert into public.work_orders(
      company_id,number,client_id,quote_id,assigned_to,status,scheduled_date,scheduled_time,
      address,service_place,request,pre_notes,pending_note,extra_cost,needs_return,warranty_id,
      origin_wo_id,is_warranty_visit,problem_report,created_by
    ) values (
      p_company,v_number,nullif(p_row->>'client_id','')::uuid,nullif(p_row->>'quote_id','')::uuid,
      coalesce(nullif(p_row->>'assigned_to','')::uuid,v_uid),
      coalesce(nullif(p_row->>'status','')::public.zt_wo_status,'unscheduled'),
      nullif(p_row->>'scheduled_date','')::date,nullif(p_row->>'scheduled_time','')::time,
      nullif(p_row->>'address',''),nullif(p_row->>'service_place',''),nullif(p_row->>'request',''),
      nullif(p_row->>'pre_notes',''),nullif(p_row->>'pending_note',''),
      coalesce((p_row->>'extra_cost')::numeric,0),coalesce((p_row->>'needs_return')::boolean,false),
      nullif(p_row->>'warranty_id','')::uuid,nullif(p_row->>'origin_wo_id','')::uuid,
      coalesce((p_row->>'is_warranty_visit')::boolean,false),nullif(p_row->>'problem_report',''),v_uid
    ) returning id into v_id;
  else
    select id into v_id
      from public.work_orders
      where id=p_wo and company_id=p_company
      for update;
    if v_id is null then raise exception 'Ordem de serviço não encontrada' using errcode='42501'; end if;
    if exists(select 1 from public.work_orders where id=v_id and status='done') then
      raise exception 'OS concluída não pode ser regravada por este fluxo' using errcode='42501';
    end if;

    update public.work_orders set
      client_id=nullif(p_row->>'client_id','')::uuid,
      quote_id=nullif(p_row->>'quote_id','')::uuid,
      assigned_to=coalesce(nullif(p_row->>'assigned_to','')::uuid,assigned_to),
      status=coalesce(nullif(p_row->>'status','')::public.zt_wo_status,status),
      scheduled_date=nullif(p_row->>'scheduled_date','')::date,
      scheduled_time=nullif(p_row->>'scheduled_time','')::time,
      address=nullif(p_row->>'address',''),
      service_place=nullif(p_row->>'service_place',''),
      request=nullif(p_row->>'request',''),
      pre_notes=nullif(p_row->>'pre_notes',''),
      pending_note=nullif(p_row->>'pending_note',''),
      extra_cost=coalesce((p_row->>'extra_cost')::numeric,0),
      needs_return=coalesce((p_row->>'needs_return')::boolean,false),
      warranty_id=nullif(p_row->>'warranty_id','')::uuid,
      origin_wo_id=nullif(p_row->>'origin_wo_id','')::uuid,
      is_warranty_visit=coalesce((p_row->>'is_warranty_visit')::boolean,false),
      problem_report=nullif(p_row->>'problem_report',''),
      updated_at=now()
    where id=v_id;
  ...
  for item in select value from jsonb_array_elements(coalesce(p_items,'[]'::jsonb)) loop
    insert into public.work_order_items(
      work_order_id,company_id,kind,service_id,product_id,name,unit,quantity,
      unit_price,unit_cost,notes,is_extra,price_pending
    ) values (
      v_id,p_company,
      coalesce(nullif(item->>'kind','')::public.zt_item_kind,'free'),
      nullif(item->>'service_id','')::uuid,
      ...
      coalesce((item->>'price_pending')::boolean,false)
    );
  end loop;
...
begin
  ...
  if p_purchase is null then
    ...
    select id,entry_id,number into v_id,v_entry,v_number
    ...
    update public.purchases set
      supplier_name=v_supplier,purchase_date=v_date,payment_method=v_method,
      due_date=nullif(p_row->>'due_date','')::date,notes=nullif(p_row->>'notes',''),updated_at=now()
    where id=v_id;
  ...
  for item in select value from jsonb_array_elements(p_items) loop
    ...
    values(
      v_id,p_company,nullif(item->>'product_id','')::uuid,
      left(coalesce(nullif(item->>'name',''),'Item'),500),
  ...
  if v_entry is null then
    ...
    update public.financial_entries set
      description=left('Compra '||v_number||' · '||v_supplier,500),
      amount=v_total,due_date=v_due,paid=v_paid,
      ...
      category='Materiais',purchase_id=v_id
...
create or replace function public.zt_save_quote(
  p_company uuid,p_quote uuid,p_row jsonb,p_items jsonb default '[]'::jsonb
) returns uuid
language sql
security definer
set search_path=zt_private
as $$ select zt_private.zt_save_quote(p_company,p_quote,p_row,p_items); $$;

create or replace function public.zt_save_work_order(
  p_company uuid,p_wo uuid,p_row jsonb,p_items jsonb default '[]'::jsonb
) returns uuid
language sql
security definer
set search_path=zt_private
as $$ select zt_private.zt_save_work_order(p_company,p_wo,p_row,p_items); $$;

create or replace function public.zt_save_purchase(
  p_company uuid,p_purchase uuid,p_row jsonb,p_items jsonb default '[]'::jsonb
) returns uuid
language sql
security definer
set search_path=zt_private
as $$ select zt_private.zt_save_purchase(p_company,p_purchase,p_row,p_items); $$;
...
grant execute on function public.zt_save_quote(uuid,uuid,jsonb,jsonb) to authenticated;
grant execute on function public.zt_save_work_order(uuid,uuid,jsonb,jsonb) to authenticated;
grant execute on function public.zt_save_purchase(uuid,uuid,jsonb,jsonb) to authenticated;

revoke all on function zt_private.zt_save_quote(uuid,uuid,jsonb,jsonb) from public,anon,authenticated;
revoke all on function zt_private.zt_save_work_order(uuid,uuid,jsonb,jsonb) from public,anon,authenticated;
