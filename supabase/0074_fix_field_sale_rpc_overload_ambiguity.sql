-- ZiisTec · 0074 — remove ambiguidade entre RPC legado (6 args) e novo fluxo (8 args)
-- Mantém o RPC legado para compatibilidade/testes e exige argumentos explícitos no RPC novo.

-- Defaults em p_client/p_service_place faziam chamadas de 6 argumentos ficarem ambíguas.
-- Removemos somente a assinatura de 8 argumentos e a recriamos sem defaults.
drop function if exists public.zt_sell_product_direct(uuid,uuid,numeric,text,text,uuid,uuid,text);

create function public.zt_sell_product_direct(
  p_company uuid,
  p_product uuid,
  p_quantity numeric,
...
begin
  ...
  if p_client is not null then
    ...
    ) then raise exception 'Cliente inválido para esta empresa' using errcode='42501'; end if;
